...
# TCGAnalyze_DEA identified 3,390 differentially expression genes (DEG) (log fold change >=1 and FDR < 1%) between 114 normal and 1097 BRCA samples. 
# In order to understand the underlying biological process from DEGs we performed an enrichment analysis 
# using TCGAnalyze_EA_complete function.
...
# TCGAbiolinks outputs bar chart with the number of genes for the main categories of three ontologies:
...
# The Kaplan-Meier analysis was used to compute survival univariate curves
# and log-Ratio test was computed to assess the statistical significance by using TCGAanalyze_SurvivalKM function
# Starting with 3,390 DEGs genes we found 555 significantly genes with p.value <0.05.
...
# Cox-regression analysis was used to compute survival multivariate curves
# Cox p-value was computed to assess the statistical significance by using TCGAnalyze_SurvivalCoxNET function. 
# Survival multivariate analysis found 160 significantly genes according to the cox p-value FDR 5.00e-02. 
# From DEGs that we found to correlate significantly with survival by both univariate and multivariate analyses we analyzed the following network.

# The interactome network graph was generated using STRING.,org.Hs.string version 10 (Human functional protein association network). 
# The network graph was resized by dnet package considering only multivariate survival genes, 
# with strong interaction (threshold = 700) we obtained a subgraphsub graph of 24 nodes and 31 edges.
...
# We focused on the analysis of LGG samples. 
# In particular, we used TCGAbiolinks to download 293 samples with molecular subtypes. 
...
lgg.exp <- GDCprepare(
  query = query.exp, 
  save = TRUE, 
  save.filename = "lggExp.rda"
)

# First, we searched for possible outliers using the TCGAanalyze_Preprocessing function,
# which performs an Array Array Intensity correlation AAIC. 
# We used all samples in expression data which contain molecular subtypes, filtering out samples without molecular information, and using only IDHmut-codel (n=85), IDHmut-non-codel (n=141) and IDHwt (n=56), NA (11), to define a square symmetric matrix of pearson correlation among all samples (n=293). 
# According to this matrix we found no samples with low correlation (cor.cut = 0.6) that can be identified as possible outliers, 
# so we continued our analysis with 70 samples.

# Second, using the TCGAanalyze_Normalization function we normalized mRNA transcripts and miRNA, using EDASeq package.
# This function does use Within-lane normalization procedures to adjust for GC-content effect (or other gene-level effects) on read counts: 
# loess robust local regression
# global-scaling
# full-quantile normalization (Risso et al. 2011) 
...
# Then we applied two Hierarchical cluster analysis on 1187 mRNAs after the three filters described above
# The first cluster using as method ward.D2, and the second with ConsensusClusterPlus.
...
# The next steps will be to visualize the data. 
# First, we created the survival plot.
...
# Finally, we will take a look in the mutation genes. 
# We will first download the MAF file with GDCquery_Maf. 
...
# In recent years, it has been described the relationship between DNA methylation and gene expression 
# The study of this relationship is often difficult to accomplish.

# This case study will show the steps to investigate the relationship between the two types of data.

# First, we downloaded ACC DNA methylation data for HumanMethylation450k platforms
# and ACC RNA expression data for Illumina HiSeq platform.
...
# So, we selected the groups CIMP-low and CIMP-high to do RNA expression and DNA methylation comparison.
...
# For DNA methylation, we perform a DMC (different methylated CpGs) analysis
# Gives the difference of DNA methylation for the probes of the groups and their significance value. 
...
# For the expression analysis, we do a DEA (differential expression analysis) 
# Gives the fold change of gene expression and their significance value.
...
# An example of package to perform DNA methylation and RNA expression analysis is ELMER (L. Yao et al. 2015, @elmer2, @yao2015demystifying). 
# ELMER combines DNA methylation and gene expression data from human tissues to infer multi-level cis-regulatory networks. 
# ELMER uses DNA methylation to identify distal probes, and correlates them with the expression of nearby genes to identify one or more transcriptional targets. 
# Transcription factor (TF) binding site analysis of those anti-correlated distal probes is coupled with expression analysis of all TFs to infer upstream regulators. 
...
# This function will keep only samples that have both DNA methylation and gene expression.
...
# From this analysis it is possible to verify the relationship between nearby 20 gene expression vs DNA methylation at this probe. 
# The result of this is show by the ELMER scatter plot.
...
# You can see the anticorrelated pairs of gene and probes by drawing a heatmap.
